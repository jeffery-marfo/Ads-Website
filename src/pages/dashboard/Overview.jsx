...
const Overview = () => {
  ...
  const displayDateRange = `${formatDate(dateRange.startDate)} - ${formatDate(dateRange.endDate)}`;
  ...
  useEffect(() => {
    const generateData = () => {
      ...
      return dates.map(date => {
        ...
        const baseClicks = Math.floor(baseImpressions * (Math.random() * 0.15 + 0.05));
        const baseConversions = Math.floor(baseClicks * (Math.random() * 0.08 + 0.02));
    ...
    const totalImpressions = newData.reduce((sum, day) => sum + day.impressions, 0);
    const totalClicks = newData.reduce((sum, day) => sum + day.clicks, 0);
    const totalConversions = newData.reduce((sum, day) => sum + day.conversions, 0);
    
    setStats([
      { label: "Total Impressions", value: totalImpressions.toLocaleString(), color: "blue" },
      { label: "Total Clicks", value: totalClicks.toLocaleString(), color: "red" },
      { label: "Total Conversions", value: totalConversions.toLocaleString(), color: "green" }
    ]);
  }, [dateRange]);
  ...
  return (
    <div className="flex flex-col p-4 md:p-6 ml-60">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm md:text-base">Manage your ads and track performance with ease!</p>
          </div>
        </div>
        
        <div className="relative">
          <div className="flex items-center border rounded-md px-3 py-2 cursor-pointer hover:bg-gray-50"
        ...
        <div className="h-64 md:h-80">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Line type="monotone" dataKey="clicks" stroke="#ef4444" strokeWidth={2} dot={{ r: 3 }} />
          ...
          {activities.map(activity => (
            <div key={activity.id} className="p-4">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-1">
                ...
                <span className="text-sm text-gray-500 md:ml-4">{activity.time}</span>
              </div>
            </div>
          ))}
